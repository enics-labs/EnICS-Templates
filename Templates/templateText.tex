% This file contains the guidance text that is pulled into the templates with \guide{tag}.
% Each block is delimited by %<*tag> ... %</tag> (catchfilebetweentags package).
% DO NOT write your manuscript in this file! Everything here disappears from your PDF
% as soon as you set \guidefalse in your preamble.
%
% IMPORTANT: keep each paragraph on ONE long line. The catchfilebetweentags package
% does not turn the end of a line into a space, so text split over several lines comes
% out with the words run together ("the project.If you are reading this").

% Important notes shown at the top of every paper template
%<*notes>
\red{\noindent\textbf{\textsc{Important Notes Before We Start}}:
    \begin{itemize}
        \item This file should be your \texttt{main.tex} in the root of the project. If you are reading this from inside the \texttt{Templates} folder, move it there first (delete the old \texttt{main.tex}, move this file to the root and rename it \texttt{main.tex}). Or just ask Claude to ``start a new EnICS manuscript''.
        \item All of the colored guidance text (like this) is controlled by \textbf{one switch}: change \texttt{\textbackslash guidetrue} to \texttt{\textbackslash guidefalse} in the preamble and all of it disappears from the PDF. You can then delete the \texttt{\textbackslash guide\{...\}} lines and the \texttt{guidance} blocks at your leisure, and delete the \texttt{Templates} folder altogether.
        \item Put your paper-specific acronyms and newcommands inside \texttt{newcommands/this\_glossary.tex}.
        \item Put your paper-specific references inside \texttt{bibliography/this\_bibliography.bib}.
        \item This project was started from EnICS Templates version \enicstemplateversion.
    \end{itemize}
}
%</notes>
...
%<*abstract>
\color{teal}
    This is a template for IEEE manuscripts. It includes a brief description of some input files useful to write articles for conferences or journals. In this space, you will want to write your abstract. The abstract should be an ``executive summary'' of your paper in about 5--6 sentences.  Follow the flow of your paper. First sentence or two should be introductory and motivation. Then a couple of sentences about the proposed solution. Finish with a conclusive sentence stating a few important results (NUMBERS!) from your paper. \blue{A good suggestion is to write your abstract at the end, after finishing the rest of the paper!} Here is how a citation looks~\cite{amit2017idsramTVLSI}.
\color{black}
%</abstract>

% This tells you what to write in the Acknowledgments
%<*acknowledgments>
 \color{teal}
...
\color{teal}
    ...
    \indent By the third paragraph, you have to specifically state the problem that your work will try to solve. Describe the problem and why it is hard and/or important to fix it. It is not a bad idea to add a general figure here -- something that could appear on the first page and provide an overview of the big picture application that you are dealing with. Not mandatory, but nice to have some visualization early on in your paper.\\
...
\color{teal}
    ...
    \begin{itemize}
        ...
        \item You can crop the figures with the trim commands.
        \item Sub-figures are made with the \texttt{subfig} package: \texttt{\textbackslash subfloat[caption]\{\textbackslash includegraphics\{...\}\}} inside a \texttt{figure}.
        \item If you cannot pass \texttt{pdf\_express}, use the ``\texttt{latexmkrc}'' file, which will embed all the fonts. For more, refer to: \href{https://www.overleaf.com/learn/latex/Articles/How\_to\_use\_latexmkrc\_with\_Overleaf}{\blue{How to use latexmkrc with Overleaf}} or \href{https://www.overleaf.com/learn/latex/Questions/My\_submission\_was\_rejected\_by\_the\_journal\_because\_\%22Font\_XYZ\_is\_not\_embedded\%22.\_What\_can\_I\_do\%3F}{\blue{My submission was rejected by the journal because ``Font XYZ is not embedded''}}.
    \end{itemize}
...
% This tells you what to write in the Abstract of a research proposal
%<*abstractRP>
\color{teal}
    This is a template for BIU Research Proposals. It includes a general outline for a research proposal that could be followed. The Hebrew front page and the Hebrew abstract are typeset from the \texttt{.tex} files under \textbf{AuxiliaryPages} when you compile with \textbf{LuaLaTeX} (on Overleaf: Menu, Compiler, LuaLaTeX). If you must use pdfLaTeX, update the \texttt{.docx} files there instead, save them as PDF and point the \texttt{\textbackslash includepdf} commands at them.\\
...
% This tells you what to write in the Abstract of a thesis
%<*abstractThesis>
\color{teal}
    This is a template for writing a Thesis. It includes a general outline for a thesis that could be followed. The English and Hebrew front pages and the Hebrew abstract are typeset from the \texttt{.tex} files under \textbf{AuxiliaryPages} when you compile with \textbf{LuaLaTeX} (on Overleaf: Menu, Compiler, LuaLaTeX). Fill in the title, author, advisor and date macros at the top of \texttt{main.tex}. If you must use pdfLaTeX, update the \texttt{.docx} files there instead, save them as PDF and point the \texttt{\textbackslash includepdf} commands at them.\\
    Note that the abstract for a thesis or research proposal is much more extensive than the short abstract for an article. It should be approximately 1 page long and really be an executive summary of the proposal, including some background, presenting the problem, and explaining how you propose to address it. Don't forget to point out your achievements, such as the number of papers that you published.
\color{black}
%</abstractThesis>

% This tells you what to write in the Objectives
%<*objectives>
\color{teal}
...
%<*methodology>
\color{teal}
The methodology for carrying out this research is ... \blue{It is good advice to use a flowchart to help describe the methodology.}
\color{black}
%</methodology>

% Chapter structure hint for theses
%<*thesisContent>
\blue{From this chapter and on, you should present the content of your thesis. Note that it is often nice to include each chapter as a separate .tex file, using the \textbackslash include\{filename\} option. You can reduce the compile time of your paper by adding \textbackslash includeonly\{filename\} in the options. This will only output the content of the selected file, but will retain the full file's numbering and everything.}
%</thesisContent>
